...
import com.login.news.api.ApiClient
...
class TambahBeritaActivity : AppCompatActivity() {
    ...
    private fun tambahBerita(judul: String, fileGambar: File, isiBerita: String) {
        ...
        val decriptionBerita = isiBerita.toRequestBody("text/plain".toMediaTypeOrNull())

        ApiClient.apiService.addBerita(title, decriptionBerita, partFileGambar).enqueue(object :
            Callback<TambahBeritaResponse>{
